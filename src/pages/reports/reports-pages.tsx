...
function ReportPagination(props: {
  page: number
  total: number
...
const salesColumns: TableColumnsType<{
  ...
  },
  {
    ...
    key: 'completed',
    sorter: (a, b) =>
      dealCompletionIso(a.deal).localeCompare(dealCompletionIso(b.deal)),
    render: (_, row) =>
      row.deal.completedAt ? formatDateRu(row.deal.completedAt) : '—',
  },
]
...
const stagesColumns: TableColumnsType<{
...
}> = [
  {
    ...
    render: (s: DealStatus) => (
      <StageStatusCell $status={s}>
        {DEAL_STATUS_META[s]?.label ?? s}
      </StageStatusCell>
    ),
  },
  {
...
export function ReportsSalesPage() {
  ...
  const stagesRows = useMemo(() => {
    ...
    for (const d of deals) {
      if (!isoTimestampInRange(d.createdAt, stagesRange.start, stagesRange.end))
        continue
